...
mount_partition()
{
        ...
        if [ ! -f /etc/selinux/config ] ; then
            mount -i -t auto "/dev/$1" "${destdir}" -o uid=0,gid=1015,umask=002,nodev,noexec,nosuid
        else
            if ! mount -i -t auto "/dev/$1" "${destdir}" -o uid=0,gid=1015,umask=002,context=system_u:object_r:sdcard_t:s0,nodev,noexec,nosuid; then
                # failed to mount
                exit 1
            fi
        fi
